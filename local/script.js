...
var updateImage = function($http, server) {
  $http.get(base + "proxy/namespaces/kubeapp/pods/" + server.podName + "/data.json")
...
var updateServer = function($http, server) {
  $http.get(base + "namespaces/kubeapp/pods/" + server.podName)
    .success(function(data) {
      console.log(data);
...
var isUpdateDemoPod = function(pod) {
    return pod.metadata && pod.metadata.labels && pod.metadata.labels.name == "pf9";
};
...
var update = function($scope, $http) {
  ...
  $http.get(base + "namespaces/kubeapp/pods")
    .success(function(data) {
      console.log(data);
      var newServers = [];
